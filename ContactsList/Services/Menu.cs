...
internal class Menu
{
    ...
    private void OptionFive()
    {
        ...
        Console.WriteLine("1. Företagskund");
        Console.WriteLine("2. Privatekund");
        Console.Write("Välj vad för typ av kund du vill ta bort: ");
        var option = Console.ReadLine();

      
        switch (option)
        {
            case "1": RemoveBusinessCustomer(); break;
            case "2": RemovePrivateCustomer(); break;
        }
    ...
    private void RemoveBusinessCustomer()
    {
        Console.WriteLine("All customers are listed below: ");
        ...
        Console.WriteLine("Skriv in företagsnamn: ");
        var removeBusinessCustomer = Console.ReadLine() ?? "";
        var index = contactList.FindIndex(x => x.CompanyName == removeBusinessCustomer);


        if (removeBusinessCustomer != "")
        {

            if (index >= 0)
            {
                Console.WriteLine($"Are you sure you want to delete{removeBusinessCustomer} ? (Y/N)");
                Console.WriteLine("");
                if (Console.ReadKey().Key == ConsoleKey.Y)
                {
                    contactList.RemoveAt(index);
                    file.Save(JsonConvert.SerializeObject(contactList));
                    Console.WriteLine($"\n{removeBusinessCustomer} was successfully removed from the list!");
                    Console.ReadKey();
                }
